...
function showCustomNotify(Message,Type,Position) {
    thisNotify = UIkit.notify({
        message: Message,//$element.data('message') ? $element.data('message') : '',
        status: Type,//$element.data('status') ? $element.data('status') : '',
        timeout: 5000, //$element.attr('data-timeout') ? $element.data('timeout') : 5000,
        group: null,//$element.data('group') ? $element.data('group') : null,
        pos: Position,//$element.data('pos') ? $element.data('pos') : 'top-center',
        onClose: function () {
            $body.find('.md-fab-wrapper').css('margin-bottom', '');
            //if ($element.data('callback')) {
            //    executeCallback($element.data('callback'));
